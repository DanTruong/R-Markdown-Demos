...
```{r, echo=FALSE}
storm.data.final <- read.csv("stormDataFinal.csv")
library(data.table)
library(ggplot2)
injuries <- data.table(Storm = storm.data.final$Storm, Injuries = storm.data.final$Injuries)
deaths <- data.table(Storm = storm.data.final$Storm, Deaths = storm.data.final$Fatalities)
casualties <- data.table(Storm = storm.data.final$Storm, Totals = deaths$Deaths + injuries$Injuries)
summary <- data.frame(head(casualties[order(-casualties$Totals), ], 10))
```


```{r, echo=FALSE}
# Pie chart of summary
pie <- ggplot(summary, aes(x="", y=Totals, fill=Storm)) + geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) + ggtitle("Deadliest Events")
pie
# Print summary of casualties
summary
```
...
```{r, echo=FALSE}
# Calculate monetary damages from Property and Crop damages
damages <- data.table(Storm = storm.data.final$Storm, USD = storm.data.final$Property.Damage + storm.data.final$Crop.Damage)

# Calculate top 10 storms with the most damages
summary <- data.frame(head(damages[order(-damages$USD), ], 10))

# Pie chart of summary
pie <- ggplot(summary, aes(x="", y=USD, fill=Storm)) + geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) + ggtitle("Costliest Events")
pie
